 ...
 *
 * Two uses, same class:
 *   1. Hero X-ray lens — REPLACES the hero's existing 2D `dustRef` canvas.
 *      Mount scoped to the hero section's bounding rect.
 *   2. Sitewide cursor trail — mount ONE instance full-viewport (fixed
 *      canvas covering the whole page) so the effect works everywhere you
 *      move the cursor, not just in the hero. Use setSuppressed(true) over
 *      text-bearing sections so it doesn't reintroduce the legibility
 *      problems already solved elsewhere on the site (see setSuppressed
 *      below) — see dot-trail-sitewide-test.html for the reference wiring.
 *
 * Renders with a transparent background (alpha-blended) so it composites
 * directly over whatever's already on the page — this is not a full-bleed
 * background effect, only a circular reveal + trail.
 *
 ...
 *   lens.setCursor(xPx, yPx, isInside);   // call on pointermove
 *   lens.setSuppressed(overTextZone);     // call when cursor enters/leaves a no-trail zone
 *   lens.tick(dtSeconds);                 // call once per shared rAF frame
 *   lens.setActive(false);                // call when off-screen — stops sim, does not destroy
 ...
 *   if (!isTouchOrNarrow) { this.dotFieldLens = new DotFieldLens(canvasEl); }
...
export class DotFieldLens {
  constructor(canvas, options = {}) {
    ...
    this.active = true;
    this.suppressed = false; // true while cursor is over a registered no-trail zone
  ...
  _compile() {
    ...
    const RENDER_FS = `#version 300 es
      ...
      uniform float u_lensActive;  // 0/1 — cursor currently inside the tracked area
      uniform float u_suppressed; // 0/1 — cursor is over a registered no-trail zone (e.g. body text)
      ...
      void main() {
        ...
        float visibility = max(lensMask, trail) * (1.0 - u_suppressed);
    ...
    this.renderU = {
      ...
      suppressed: gl.getUniformLocation(this.renderProgram, 'u_suppressed'),
    };
  }
  ...
  /**
   * Call whenever the cursor enters/leaves a registered no-trail zone
   * (e.g. body-copy sections). While suppressed, nothing is drawn AND no
   * new trail is painted — so the trail doesn't silently build up out of
   * sight and then "pop in" bright the moment you leave the zone.
   */
  ...
  tick(dt) {
    ...
    const hasCursor = (this.cursorInside && !this.suppressed) ? 1.0 : 0.0;
    ...
    gl.uniform1f(this.renderU.lensActive, (this.cursorInside && !this.suppressed) ? 1.0 : 0.0);
    gl.uniform1f(this.renderU.suppressed, this.suppressed ? 1.0 : 0.0);
    gl.drawArrays(gl.TRIANGLES, 0, 3);
  }
