...
export class DotImageLens {
  constructor(canvas, image, options = {}) {
    ...
    this.cfg = {
      ...
      // hero: true — the lens is a magnifier you can hold still.
      // sitewide: false — visibility comes only from the decaying trail.
      showIdleLens:   options.showIdleLens    ?? true,
  ...
  tick(dt) {
    ...
    for (let y = 0; y < this.rows; y++) {
      for (let x = 0; x < this.cols; x++) {
        ...
        this.revealGrid[idx] = cfg.showIdleLens ? Math.max(val, lensMask) : val;
      }
    }
